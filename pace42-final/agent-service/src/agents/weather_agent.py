...
class WeatherAgent:
    ...
    async def get_running_conditions(self, latitude: float, longitude: float) -> Dict[str, Any]:
        ...
        # Simple bar chart for running conditions
        chart = {
            "id": "weather_conditions",
            "type": "bar",
            "title": "Running Conditions by Hour",
            "xLabels": [h["time"] for h in hours[:8]],  # Show first 8 hours
            "yLabel": "Score",
            "series": [{
                "label": "Condition Score",
                "data": [h["score"] for h in hours[:8]],
                "color": "#00D4AA"
            }]
        }
