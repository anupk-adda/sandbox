...
class TokenService {
   ...
   */
  async generateTokenPair(userId: string, username: string, rememberMe: boolean = false): Promise<TokenPair> {
    ...
    databaseService.run(
      `INSERT INTO user_sessions (id, user_id, refresh_token_hash, expires_at, created_at)
       VALUES (?, ?, ?, ?, ?)`,
      [sessionId, userId, refreshTokenHash, expiresAt.toISOString(), new Date().toISOString()]
    );
  ...
  async refreshAccessToken(refreshToken: string): Promise<TokenPair | null> {
    try {
      ...
      // Find the session
      const session = databaseService.get(
   ...
   */
  async generatePasswordResetToken(userId: string): Promise<PasswordResetToken | null> {
    try {
      ...
      databaseService.run(
        `INSERT INTO password_reset_tokens (id, user_id, token_hash, expires_at, created_at)
         VALUES (?, ?, ?, ?, ?)`,
        [tokenId, userId, tokenHash, expiresAt.toISOString(), new Date().toISOString()]
      );
      ...
      databaseService.run(
        `UPDATE password_reset_tokens 
         ...
         AND used_at IS NULL 
         AND id != ?`,
        [new Date().toISOString(), userId, tokenId]
      );
  ...
  async verifyPasswordResetToken(token: string): Promise<string | null> {
    try {
      ...
      const resetToken = databaseService.get(
  ...
  async cleanupExpiredTokens(): Promise<void> {
    try {
      ...
      databaseService.run(
        `DELETE FROM password_reset_tokens WHERE expires_at < datetime('now', '-7 days')`
      );
