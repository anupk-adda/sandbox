...
router.post('/validate-garmin', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const testExchange = await garminTokenService.exchangeCredentialsForTokens(
      userId,
    ...
    // Update user record
    databaseService.run(
      'UPDATE users SET garmin_user_id = ? WHERE id = ?',
...
router.get('/me', authenticateToken, async (req: Request, res: Response) => {
  ...
  const userDetails = databaseService.get(
    'SELECT id, email, garmin_user_id, created_at FROM users WHERE id = ?',
    [user.userId]
  );
...
// Revoke Session
// ============================================
router.post('/sessions/:sessionId/revoke', authenticateToken, async (req: Request, res: Response) => {
  try {
