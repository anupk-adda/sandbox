...
router.post('/', authenticateToken, (req, res) => {
    try {
        const { goalDistance, goalDate, daysPerWeek } = req.body;
...
// Get plan summary
router.get('/:planId/summary', authenticateToken, (req, res) => {
    try {
...
// Get week details
router.get('/:planId/weeks/:weekStart', authenticateToken, (req, res) => {
    try {
        const planId = getParam(req.params.planId);
...
// Get workout detail
router.get('/workouts/:workoutId', authenticateToken, (req, res) => {
    try {
...
router.get('/users/me/prompts', authenticateToken, (req, res) => {
    try {
        const userId = req.user.userId;
        const prompts = trainingPlanService.getPromptsForUser(userId);
...
router.get('/users/:userId/prompts', authenticateToken, (req, res) => {
    try {
...
// Legacy endpoint for backward compatibility (uses userId param)
router.get('/users/:userId/active', authenticateToken, (req, res) => {
    try {
        const userId = getParam(req.params.userId);
        const planId = trainingPlanService.getActivePlanForUser(userId);
        if (!planId) {
...
//# sourceMappingURL=training-plan.routes.js.map