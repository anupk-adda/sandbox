...
const isPersonalizedQuery = (message: string): boolean => {
...
const typoNormalizationMap: Array<[RegExp, string]> = [
  [/\bfartlake\b/gi, 'fartlek'],
  [/\btemp\s+run\b/gi, 'tempo run'],
];

const normalizeTypos = (message: string): string => {
...
const isLikelyFollowUpRunning = (message: string, context: ReturnType<typeof contextManager.getContext>): boolean => {
  ...
  if (followupStarters.some(token => normalized.startsWith(token))) return true;

  const runningTerms = ['tempo', 'temp', 'fartlek', 'fartlake', 'interval', 'easy pace', 'long run', 'threshold'];
  if (runningTerms.some(term => normalized.includes(term))) return true;

  return false;
};

const hasRunningSignals = (text: string): boolean => {
...
const isCoachableQuestion = (message: string): boolean => {
  const normalized = message.toLowerCase();
  const patterns = [
    /easy pace/,
    /tempo pace/,
...
const planDrafts = new Map<string, PlanDraft>();
const unsubscribeConfirmations = new Map<string, boolean>();

const planDistanceAliases: Array<[RegExp, GoalDistance]> = [
...
const extractGoalDistance = (message: string): GoalDistance | undefined => {
...
const extractDaysPerWeek = (message: string): number | undefined => {
...
const extractGoalDate = (message: string): string | undefined => {
...
const isPlanRequest = (message: string): boolean => /\b(training plan|plan)\b/i.test(message);
...
const hasPlanFollowupSignals = (message: string, extracted: PlanDraft): boolean => {
  if (extracted.goalDistance || extracted.goalDate || extracted.daysPerWeek) return true;
  if (isPlanRequest(message)) return true;
  if (/\b(race|goal|training|days per week|day per week|week)\b/i.test(message)) return true;
  if (monthSignal.test(message)) return true;
  if (/^\s*\d+\s*(days?|x)?\s*(per\s*)?(week|wk)?\b/i.test(message)) return true;
  return false;
};
const isPlanCreationSignal = (message: string, draft: PlanDraft): boolean => {
  if (
    isShowFullPlan(message)
  ...
  if (draft.goalDistance || draft.goalDate || draft.daysPerWeek) return true;
...
const isConfirmUnsubscribe = (message: string): boolean => /\bconfirm unsubscribe\b/i.test(message) || /\byes\s*,?\s*unsubscribe\b/i.test(message);
const isCancelUnsubscribe = (message: string): boolean => /\bcancel\b/i.test(message) || /\bkeep\b/i.test(message) || /\bno\b/i.test(message);

const buildPlanMissingPrompt = (draft: PlanDraft): string => {
  const missing: string[] = [];
  if (!draft.goalDistance) missing.push('goal distance (5K, 10K, Half, or Marathon)');
  if (!draft.goalDate) missing.push('target race date (YYYY-MM-DD)');
  if (!draft.daysPerWeek) missing.push('days per week you can run');
...
const buildTrainingSummary = (charts?: any[]): TrainingSummary | undefined => {
  if (!Array.isArray(charts)) return undefined;
  const distanceChart = charts.find(chart =>
    String(chart?.id || '').includes('distance') || String(chart?.yLabel || '').includes('km')
  );
  const paceChart = charts.find(chart =>
    String(chart?.id || '').includes('pace') || String(chart?.yLabel || '').includes('min/km')
  );

  const labels: string[] = distanceChart?.xLabels || paceChart?.xLabels || [];
  const distances: number[] = distanceChart?.series?.[0]?.data || [];
  const paces: number[] = paceChart?.series?.[0]?.data || [];
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const extractedPlanInfo: PlanDraft = {
    ...
    const planSignalDraft: PlanDraft = {
    ...
    let classification: { intent: any; raw?: any } | null = null;
    if (!planDraft) {
    ...
    if (intent.type === 'weather') {
      ...
      contextManager.recordAnalysis(
        ...
        response.agent,
        response.response,
        response.weather ? { weather: response.weather } : undefined
      );
    ...
    if (intent.requiresGarminData) {
      ...
      try {
        ...
        if (isWeatherQuery && context.location?.latitude !== undefined && context.location?.longitude !== undefined) {
          const weather = await agentClient.getRunningConditions(
