...
type ProficiencyLabel = 'beginner' | 'intermediate' | 'advanced';
...
class PersonaAgent {
  ...
  private getProfileRecord(conversationId: string): PersonaProfileRecord | undefined {
    this.ensureSchema();
    return databaseService.get<PersonaProfileRecord>(
      `SELECT conversation_id, profile_json, runner_history, runner_proficiency,
              proficiency_score, last_refreshed_at, proficiency_updated_at,
              created_at, updated_at
       FROM persona_profiles WHERE conversation_id = ?`,
      [conversationId]
    );
  }

  private upsertProfileRecord(conversationId: string, data: Partial<PersonaProfileRecord>): void {
    this.ensureSchema();
  ...
  private scoreProficiencyFromQuestions(questions: string[]): { score: number; tags: string[] } {
  ...
  private labelForScore(score: number): ProficiencyLabel {
    if (score >= 70) return 'advanced';
    if (score >= 40) return 'intermediate';
    return 'beginner';
  }

  private deriveStatsFromCharts(charts: any[]): {
    avg_pace_min_per_km?: number;
    avg_hr?: number;
    avg_cadence?: number;
    avg_distance_km?: number;
    ...
    if (!Array.isArray(charts)) return stats;
    ...
    for (const chart of charts) {
      if (!chart || !Array.isArray(chart.series)) continue;
      for (const series of chart.series) {
        const raw = Array.isArray(series.rawData) ? series.rawData : [];
        const avg = numericAverage(raw.filter((v: any) => typeof v === 'number'));
  ...
  updateRunnerProficiency(conversationId: string): void {
    this.ensureSchema();
    const rows = databaseService.all<{ content: string }>(
  ...
  async refreshRunnerHistoryIfStale(conversationId: string): Promise<void> {
    this.ensureSchema();
    const existing = this.getProfileRecord(conversationId);
  ...
  getProfileContext(conversationId: string): RunnerProfileContext | undefined {
    ...
    let runnerHistory: any | undefined;
  ...
  getLatestProfileContext(): RunnerProfileContext | undefined {
    this.ensureSchema();
    const record = databaseService.get<PersonaProfileRecord>(
      `SELECT conversation_id, profile_json, runner_history, runner_proficiency,
              proficiency_score, last_refreshed_at, proficiency_updated_at,
              created_at, updated_at
